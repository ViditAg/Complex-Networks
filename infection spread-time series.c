...
float total_infect, total_s, total_r,avg_total_infect,avg_total_s,avg_total_r;
int inf_nb[m][n];
int tau_O,tau_i;
int site;
tau_O = 13;
...
srand(time(0));
FILE *fp;  
fp = fopen("infect_spread40x40_time-series.txt","wt");
neta = 0.0;
...
avg_total_s += total_s/(n*m);
avg_total_r += total_r/(n*m);
printf("%d %f %f %f\n",t,avg_total_infect,avg_total_s,avg_total_r);
fprintf(fp,"%d %f %f %f\n",t,avg_total_infect,avg_total_s,avg_total_r);
}
fclose(fp);
printf("done");
