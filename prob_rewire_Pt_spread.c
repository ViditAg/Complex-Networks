...
int main(void)
{
  ...
  float pr,prewire;       
  int count;
  float Ptchaos;                                                     // initializing network rewiring probability
  FILE *fp;  
  fp = fopen("prob_rewire_Pt_spread.txt","wt");
  srand(time(0));                                                                //random seeding to be depended on time
for(k=0;k<1000;k++)                                                               // no. of re-runs of the system
{
prewire = 1.0;                                                                 // intializing random linking probability
   E = 0.65;                                                                      // initializing coupling strength
   Ptchaos = 0.0;
   prob = 0.65;    
   count = 0;
 for(i=0;i<n;i++)               
